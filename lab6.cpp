...
{
    if(farm[x][y]=='.'){          //sunflower to zombie
        farm[x][y]='x';
    }
    else if(farm[x][y]=='#'){      //nut to damaged nut
        farm[x][y]='+';
    }
    else if(farm[x][y]=='+'){       //damged nut remains unchanged
        farm[x][y]='+';
    }
}
...
void infect_farm(char farm[][10], int x, int y, int t, int M, int N, int T){
    ...
    //now consider the boundary case when the initial location of a zombie is a cell with a nut
    else if(farm[x][y]=='#'){
        if (t==0){
            farm[x][y]='x';
            t+=1;
            infect_farm(farm,x+1,y,t,M,N,T);
            infect_farm(farm,x,y+1,t,M,N,T);
            infect_farm(farm,x-1,y,t,M,N,T);
            infect_farm(farm,x,y-1,t,M,N,T);            
        }
        else if(t>0){
